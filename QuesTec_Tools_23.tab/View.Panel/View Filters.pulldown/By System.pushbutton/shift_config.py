...
def remove_system_type_filters():
    ...
    try:
        ...
        for filter_element in all_filters:
            ...
            # Check if filter name starts with "Pipe System -" or "Duct System -" (our naming convention)
            if filter_name.startswith("Pipe System - ") or filter_name.startswith("Duct System - "):
        ...
        for filter_element in filters_to_remove:
            try:
                ...
                # Delete filter from document
                try:
                    doc.Delete(filter_id)
                    print("Deleted filter from document: " + filter_name)
                except Exception as e:
                    print("Error deleting filter from document: " + filter_name + " - " + str(e))
